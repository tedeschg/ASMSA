...
def asmsa_ae(n_features, latent_dim=2, activation="gelu"):
    ...
    def asmsa_block(x, neurons, name_prefix, use_bn=True):
    ...
    x = asmsa_block(enc_input_layer, 128, "enc_1", use_bn=False)
    x = asmsa_block(x, 64, "enc_2", use_bn=False)
    x = asmsa_block(x, 32, "enc_3", use_bn=False)
    ...
    x = asmsa_block(dec_input_layer, 32, "dec_1",use_bn=False)
    x = asmsa_block(x, 64, "dec_2", use_bn=False)
    x = asmsa_block(x, 128, "dec_3", use_bn=False)
