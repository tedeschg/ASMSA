...
def build_asmsa_beta_vae(n_features, latent_dim=2, activation="gelu",
                         recon_loss_weight=1.0, beta=0.1):  # ← CAMBIATO: beta default 0.1
    # === Encoder ===
    ...
    class BetaVAE(models.Model):
        ...
        def train_step(self, data):
            ...
            with tf.GradientTape() as tape:
                ...
                # ← CAMBIATO: Loss combinata MSE + MAE per migliore qualità visiva
                ...
                # KL divergence con clamping più conservativo
        ...
        def test_step(self, data):
            ...
            # ← CAMBIATO: Stessa loss combinata anche per validation
            mse = tf.reduce_mean(tf.square(x - reconstruction))
            mae = tf.reduce_mean(tf.abs(x - reconstruction))
            recon_loss = (0.7 * mse + 0.3 * mae) * self.recon_loss_weight
    ...
    # Inizializzazione migliore per evitare posterior collapse
    def init_encoder_weights(encoder):
        for layer in encoder.layers:
            if hasattr(layer, 'name') and layer.name:
                if 'z_log_var' in layer.name and hasattr(layer, 'bias'):
                    # ← CAMBIATO: Inizializza a -1 invece di -2 per più varianza
                    layer.bias.assign(tf.constant([-1.0] * layer.units, dtype=tf.float32))

    init_encoder_weights(encoder)
